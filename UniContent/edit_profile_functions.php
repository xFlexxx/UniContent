...
function findPublications($searchNames, $fio = '', $papersUrl = 'papers.html')
{
    ...
    $namePatterns = array_map(function ($name) {
        return '(' . preg_quote(trim($name), '/') . ')';
    }, $searchNames);

    $nameRegex = "/([A-Za-z0-9.,а-яА-Я]*)(" . implode('|', $namePatterns) . ")([A-Za-z0-9.,а-яА-Я]*)/ui";
...
function handleProfileForm($profiles, $profileFile = null)
{
    ...
    if ($profileFile && isset($profiles[$profileId]) && !$photoPath) {
        $photoPath = $profiles[$profileId]['photo'] ?? null;
    }
    ...
    return ['success' => true, 'message' => 'Профиль успешно сохранён, HTML страница обновлена'];
}
